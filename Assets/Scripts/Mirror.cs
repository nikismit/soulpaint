...
public class Mirror : MonoBehaviour {
   ...
   [SerializeField] VRAvatarController vrsetup;
    ...
    private void Setup()
    {
        if (vrsetup.actualAvatarVRIK != null)
        {
            ik = vrsetup.actualAvatarVRIK;
            mirroredAvatarRoot = GameManager.Instance.finalPuppet.transform;
        }
        ...
        float scale = GameManager.Instance.savedScale;
        mirroredAvatarRoot.localScale = new Vector3(-scale, scale, scale);
