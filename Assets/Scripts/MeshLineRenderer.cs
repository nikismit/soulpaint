...
public class MeshLineRenderer : MonoBehaviour
{
	...
	public void setWidth(float widthRec)
	{
		//lineSize = width;
		width = widthRec; 
	}

	public void AddPoint(Vector3 point, float triggerVal)
	{
		if (s != Vector3.zero)
		{
			//	lineSize =  width * triggerVal ;
			lineSize = width * (1 + .2f* (1 +  Mathf.Sin(Time.time)));
